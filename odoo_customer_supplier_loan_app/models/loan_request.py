...
class LoanRequest(models.Model):
	...
	name = fields.Char(string="Number", help="Auto Generated when the loan is submitted", readonly=True)
	partner_id = fields.Many2one('res.partner',string="Partner",required=True,domain=[('is_a_customer','=',True)])
	applied_date = fields.Date(string="Applied Date",default=fields.Datetime.now,required=True)
	loan_type_id = fields.Many2one('loan.type',string="Loan Type",required=True)
	approve_date = fields.Date(string="Approve Date", required=True)
	...
	loan_partner_type = fields.Selection([('customer','Customer'),('vendor','Supplier')],string="Loan Partner Type",default='customer')
	loan_source_id = fields.Many2many('loan.source', relation='loan_request_loan_source_rel', string='Loan Source')
	...
	duration_months = fields.Integer(string="Duration(Months)",required=False,default=1)
	duration_hours = fields.Integer(string="Duration(Hours)", required=False, default=1)
	duration_days = fields.Integer(string="Duration(Days)", required=False, default=1)

	# Related field to inpiut selected repayment period type - https://www.odoo.com/forum/help-1/how-to-apply-attrs-for-many2one-fields-142983
	repayment_period_relation = fields.Selection(related="loan_type_id.repayment_duration_type", string="Repayment Period Relation")

	loan_limit_available = fields.Integer(string="Available Loan Limit")
	...
	policy_ids = fields.Many2many('loan.policies', 'rel_policies_loan_req', string="Loan Policies",
					 related="loan_type_id.loan_policies_ids")
	...
	def _check_partner(self):
		if self.partner_id not in self.loan_type_id.partner_ids :
			raise ValidationError(_("Customer is not allowed to request for this loan type. "
									"Please Approve the customer and set loan types that the customer can access."))
		else: 
			return
		return
	...
	def action_confirm(self):
		# check loan limit
		loan_limit = self.partner_id.loan_limit
		if loan_limit <= 0.0:
			raise UserError(_('You are not eligble for loans at the moment. Please contact your administrator for more details.'))
		if self.principal_amount > loan_limit:
			raise UserError(_('The amount requested is more than applicant loan limit. Please contact your administrator more details.'))
		if not self.policy_ids:
			raise UserError(_('Please configure or add Loan Policies for applicants..!'))
		if self.policy_ids :
			max_value = 0.0
			value_gap = 0
			is_max = False
	...
	def reset_draft(self):
		self.write({'state':'draft','is_compute': False,'total_loan': 0.0,'total_interest':0.0,'balance_on_loan':0})
	...
	def compute_custom_loan(self):
		...
		r = self.rate / 100 / 12
		# rate per period
		rate_per_period = r / self.duration_months
		# Compute the monthly payment using the pmt function from numpy_financial.
		p = pmt(rate=r, nper=self.duration_months, pv=-self.principal_amount, when='begin')
		# Initialize the remaining loan balance to the original loan amount.
		remaining_loan_balance = self.principal_amount
		#################################
		#month_principal_amount = self.principal_amount / self.duration_months
		# compute monthly payments
		month_principal_amount = p
		#########################
		...
		for number in range(1, self.duration_months + 1):
			...
			if self.interest_mode == "flat":
				month_principal_amount = round(principal_payment, 2)
	...
	def onchange_partner(self):
		...
		if self.partner_id.loan_limit:
			self.loan_limit_available = self.partner_id.loan_limit

	@api.onchange('loan_type_id')
	def onchange_loan_type_id(self):
		self.interest_mode = self.loan_type_id.interest_mode
		# update others
		if self.loan_type_id.repayment_duration_type == 'hour':
			self.duration_hours = self.loan_type_id.loan_duration_value
		elif self.loan_type_id.repayment_duration_type == 'day':
			self.duration_days = self.loan_type_id.loan_duration_value
		else:
			self.duration_months = self.loan_type_id.loan_duration_value
		return
