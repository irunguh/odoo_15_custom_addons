...
class PropertyBuilding(models.Model):
    ...
    #method to called to return a list of available counties with houses.
    #we want to only display a list of counties with houses users can search from]
    def search_counties_with_house(self):
        #should return a dict
        #get counties with houses
        counties_with_houses = self.env['property.building'].search([])
        count_with_hse_list = []
        for c_h_s in counties_with_houses:
            count_with_hse_list.append(c_h_s.region_id.id)
        ###
        #coz of potential duplicates - let remove it - https://www.geeksforgeeks.org/python-ways-to-remove-duplicates-from-list/
        ###
        res = []
        #an intelligent way of appending lists with filters
        [res.append(i) for i in count_with_hse_list if i not in res]
        #then let read details for counties with above filtered ids
        # get counties
        counties_obj = self.env['property.region'].search([('id','in',res)])
        counties_results = []
        no = 0
        for c_l in counties_obj:
            no += 1
            counties_results.append({'id': no, 'name': c_l.name})
        ##
        #print("Debug::: Counties to List {0}".format(counties_results))
        return counties_results
    #given a county, return a list of all towns within that county with houses
    def search_towns_with_house_in_a_county(self,county):
        counties_obj = self.env['property.region'].search([('name', '=', county)])
        towns_with_houses = self.env['property.building'].search([('region_id','=',counties_obj.id)])
        towns_list = []
        for item in towns_with_houses:
            towns_list.append(item.city)
        #clean town_list
        clean_list = []
        [clean_list.append(i) for i in towns_list if i not in clean_list]
        #valid towns
        valid_towns = []
        #make a dict
        id = 0
        for item in clean_list:
            id += 1
            dict = { 'id': str(id), 'name': item }
            valid_towns.append(dict)
            #towns.append({str(id) +":"+item.city})
        #print(valid_towns)
        return valid_towns

    # given a county, return a list of all estates within that county with houses
    def search_esates_with_house_in_a_county(self, street):
        town = street # the parameter is passed is town
        #counties_obj = self.env['property.region'].search([('name', '=', county)])
        houses_in_a_city = self.env['property.building'].search([('city', '=', town)])
        #print("Debug --- Search houses in city {0}".format(street))
        street_list = []
        for item in houses_in_a_city:
            street_list.append(item.street)
            #print("Debug :: Street name {0} ".format(item.street))
        # clean town_list
        clean_list = []
        [clean_list.append(i) for i in street_list if i not in clean_list]
        # valid towns
        valid_streets = []
        # make a dict
        id = 0
    ...
    administrator_id = fields.Many2one('res.partner', string="Administrator/Contact", domain=[('is_company', '=', False)])
    ...
    def _compute_all_surface(self):
        surface = {'office': 0.0, 'meeting': 0.0, 'lobby': 0.0, 'staircase': 0.0, 'kitchens': 0.0, 'sanitary': 0.0,
                   ...
                   'archive': 0.0, 'cloakroom': 0.0, 'premises': 0.0, 'access': 0.0,'others': 0.0, }
        ...
        #for any other usage not listed
        self.surface_others = surface['others']
